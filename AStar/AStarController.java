...
class AStarController {
    ...
    AStarController (Board current_board_) {
        //Set current board
    ...
    * to be evaluated
    ...
    void get_moves () {
        ...
        if (check_bounds (above_x)) {
            ...
            above.misplaced = above.misplaced_amt();
            above.f = above.misplaced + above.level;
            //Check for solution
        ...
        if (check_bounds (below_x)) {
            ...
            below.misplaced = below.misplaced_amt();
            below.f = below.misplaced + below.level;
        ...
        if (check_bounds (left_y)) {
            ...
            left.misplaced = left.misplaced_amt();
            left.f = left.misplaced + left.level;
            if (left.is_solution ()) {
                solution = left;
            }
        ...
        if (check_bounds (right_y)) {
            ...
            right.misplaced = right.misplaced_amt ();
            right.f = right.misplaced + right.level;
        ...
        if (!open_list.isEmpty()) {
            Board next = new Board (null, spaces, 0,0); 
            next.f = 999999;
            //Find board with lowest f to be next
            for (int i = 0; i < open_list.size(); i ++) {
                if (open_list.get (i).f < next.f) {
                    next = open_list.get (i);
                }
            }
            current_board = next;
        }
    ...
    //Check to see if board already exist in open or closed  lists
    ...
    void solve () {
        int checks = 0; //Amt of open nodes checked
